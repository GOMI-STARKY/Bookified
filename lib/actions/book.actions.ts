...
export const saveBookSegments = async (bookId: string, clerkId: string, segments: TextSegment[]) => {
    try {
        ...
        for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
            ...
            const operations = batch.map(({ text, segmentIndex, pageNumber, wordCount }) => ({
                insertOne: {
                    document: {
                        clerkId,
                        bookId,
            ...
            await BookSegment.bulkWrite(operations, { ordered: false });
            console.log(`Batch ${batchIndex + 1}/${totalBatches} saved (${batch.length} segments)`);
        }
...
export const getBookContent = async (bookId: string): Promise<{ success: boolean; content?: string; error?: string }> => {
    try {
        ...
        if (!segments.length) {
            return { success: false, error: 'No book content found' };
        }
